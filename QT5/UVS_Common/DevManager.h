...
#define _string(_s) reinterpret_cast< LPCTSTR >(_s.utf16()) /* QString to LPCWSTR */
#else
...
#define _string(_s) ( _s.toLocal8Bit().data() ) /* QString to LPCSTR */
#endif

#ifdef WIN32
#define UVS_COLOR RGB(0, 0, 0)
#else
#define _countof(_x) ( sizeof(_x) / sizeof(_x[0]) )
#define UVS_COLOR NULL
typedef const uvs_color_t* COLORREF;
typedef void* HWND;
typedef uvs_rect_t RECT;
typedef RECT* LPRECT;
...
public:
    ...
    static int SetNosignalImage(LPCTSTR szFileName, uvs_draw_mode_e drawMode=uvs_draw_stretch, COLORREF bkColor=UVS_COLOR);
...
#else
    int EnumVideoFormat(uvs_video_format_t formats[], int &formatNum);
#endif
...
#else
    int GetVideoProperty(uvs_video_format_t &videoFormat);
    int SetVideoProperty(const uvs_video_format_t &videoFormat);
#endif
...
#else
    int VideoOSDControl(int osdIndex, uvs_osd_cmd_e cmd, void *param0=NULL, void *param1=NULL);
    int PreviewStart(HWND wnd);
    int PreviewStart(LPCRECT wndRect, uvs_window_e style, LPCTSTR title, LPRECT rc=NULL);
    int PreviewSetEvent(uvs_sdl_event_e eventMask, pUVSEventCallback pCB, void *pUserData=NULL);
#endif
    int PreviewSetRect(LPCRECT wndDispRect, LPCRECT videoCropRect, COLORREF bkColor=UVS_COLOR);
...
#else
    CFramePreview(HWND wnd, bool bAsync=false);
    CFramePreview(LPCRECT rect, uvs_window_e style, LPCTSTR title, bool bAsync=false, LPRECT rc=NULL);
    int PreviewSetEvent(uvs_sdl_event_e eventMask, pUVSEventCallback pCB, void *pUserData=NULL);
#endif
