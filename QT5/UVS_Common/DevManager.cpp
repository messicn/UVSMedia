...
{
    const int max_video_dev = 16;
    DeviceClose();

    if (videoIndex >= 0)
    {
        uvs_dev_info_tX dev[max_video_dev];
        int num = _countof(dev);
        int r = EnumDevice(uvs_dev_video_capture, dev, num);
        if (r != UVS_OK) return r;

        if (videoIndex < num) return DeviceOpen(dev[videoIndex]);
    }
...
#else
int CDevSDK::EnumVideoFormat(uvs_video_format_t formats[], int &formatNum)
{
    return uvs_enum_video_format(m_DevObj, formats, &formatNum);
}
#endif

int CDevSDK::SetAudioDevice(uvs_dev_type_e devType, int devIndex)
{
    const int max_audio_dev = 16;
    if (devIndex >= 0)
    {
        uvs_dev_info_tX dev[max_audio_dev];
        int num = _countof(dev);
...
#else
int CDevSDK::GetVideoProperty(uvs_video_format_t &videoFormat)
{
    return uvs_get_video_property(m_DevObj, &videoFormat);
}

int CDevSDK::SetVideoProperty(const uvs_video_format_t &videoFormat)
{
    return uvs_set_video_property(m_DevObj, &videoFormat);
}
#endif
...
#else
int CDevSDK::VideoOSDControl(int osdIndex, uvs_osd_cmd_e cmd, void *param0, void *param1)
{
    return uvs_osd_control(m_DevObj, osdIndex, cmd, param0, param1);
}

int CDevSDK::PreviewStart(HWND wnd)
{
    return uvs_start_preview(m_DevObj, wnd);
}

int CDevSDK::PreviewStart(LPCRECT wndRect, uvs_window_e style, LPCTSTR title, LPRECT rc)
{
    return uvs_rect_preview(m_DevObj, wndRect, style, title, rc);
}

int CDevSDK::PreviewSetEvent(uvs_sdl_event_e eventMask, pUVSEventCallback pCB, void *pUserData)
{
    return uvs_set_preview_callback(m_DevObj, eventMask, pCB, pUserData);
}
#endif

int CDevSDK::PreviewSetRect(LPCRECT wndDispRect, LPCRECT videoCropRect, COLORREF bkColor)
{
...
#else
CFramePreview::CFramePreview(HWND wnd, bool bAsync)
{
    m_Handle = uvs_create_frame_preview(wnd, bAsync);
}

CFramePreview::CFramePreview(LPCRECT rect, uvs_window_e style, LPCTSTR title, bool bAsync, LPRECT rc)
{
    m_Handle = uvs_create_rect_preview(rect, style, bAsync, title, rc);
}

int CFramePreview::PreviewSetEvent(uvs_sdl_event_e eventMask, pUVSEventCallback pCB, void *pUserData)
{
    return uvs_set_frame_preview_callback(m_Handle, eventMask, pCB, pUserData);
}
#endif
...
{
    int r;
    CFileRTSP *file = new (std::nothrow) CFileRTSP(szFileName, rtspPort, bHasAudio, szSessionName, &r);
    if (r == UVS_OK) return file;
...
{
    int r;
    CFileRTMP *file = new (std::nothrow) CFileRTMP(szFileName, rtmpUrl, bHasAudio, &r);
    if (r == UVS_OK) return file;
